...
if (DATA::__HasPostData()) {
    FLASH::clearFlashes();
    $testconnect = new mysqli();
    $testconnect->connect($postDbhost, $postDbuser, $postDbpass, $postDbname, $postDbport);
    $connresult = $testconnect->connect_errno;

    FLASH::checkAndAdd(array(
        # User account filterers
        'Username contains invalid characters' => !STR::__IsValidUsername($postUsername),
        ...
        'Wrong MySQL username or password! Please try again' => $connresult == 1045,
        'MySQL connection failed. Please check your MySQL host or port settings.' => $connresult == 2002
    ), 'You have successfully configured Astra Student System!', 'home', Index::__GetPage(), true);
