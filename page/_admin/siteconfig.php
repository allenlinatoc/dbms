...
if (!$config->Exists()) {
    ...
    $postRegistrationdelay = key_exists('registration_delay', $configdata) ?
            $configdata['registration_delay'] : $postRegistrationdelay;
...
if (DATA::__HasPostData()) {
    ...
    $postRegistrationdelay = DATA::__GetPOST('postRegistrationdelay', truu, true, true);
    ...
    $sysconfig->Create(DIR::$CONFIG . SYS::$CONFIG_SYSTEM_FILENAME);
    $sysconfig->Read();
    $sysconfig->Set('enable_system', $postSystemavailability)
            ->Set('enable_registration', $postUserregistration)
            ->Set('registration_delay', $postRegistrationdelay)
